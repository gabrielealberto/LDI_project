...
def load_inflation_linked_bond_types(path=CONFIG_PATH):
    ...
    if not isinstance(bonds, dict) or any(
        not isinstance(details, dict) or not details.get("type")
        for details in bonds.values()
    ):
        raise ValueError(
            "Inflation-linked bond configuration must map each ISIN to a payoff type."
        )
